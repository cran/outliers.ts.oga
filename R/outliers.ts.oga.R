...
"single_oga" <- function(yt,s=NULL){
  ...
  ##########################################################################################################
  # Initialize an indicator of whether the procedure is executed or not
  ...
  ##########################################################################################################
  # Check that yt must be a numeric vector or a ts object and try to transform
  ...
  ##########################################################################################################
  # If everything is correct, run the steps of the procedure. Otherwise, define the outputs and conclude

  if (run == "Yes"){

    ########################################################################################################
    # First step: Robust AR fitting
    ...
    ########################################################################################################
    # Second step: Detection of outliers with OGA with residuals after robust fitting and get cleaned series
    ...
    ########################################################################################################
    # Third step: Fit an ARIMA model to the cleaned series and obtain AR representation and contaminated residuals

    con_sarima <- sarima_fit(yt,yt_clean_rob)
    con_et <- con_sarima$con_et
    con_pis <- con_sarima$con_pis
    arima_fit <- con_sarima$arima_fit

    ########################################################################################################
    # Fourth step: Detection of outliers with OGA with contaminated residuals

    beg_eff <- max(c(arima_fit$arma[1]+arima_fit$arma[6]+s*(arima_fit$arma[3]+arima_fit$arma[7]),
                     arima_fit$arma[2]+arima_fit$arma[4]*s))
    oga_fit <- det_oga(yt,con_et,con_pis,beg_eff,end_eff,clean=F)
    aos_fit <- oga_fit$aos
    lss_fit <- oga_fit$lss

    ########################################################################################################
    # Fifth step: If outliers have been detected, find the final outlier configuration; otherwise, stop

    out_config_out <- config_out(nT,aos_rob,aos_fit,lss_rob,lss_fit)
    can_aos <- out_config_out$can_aos
    can_lss <- out_config_out$can_lss
    n_can_aos <- length(can_aos)
    n_can_lss <- length(can_lss)
    n_can_out <- n_can_aos + n_can_lss

    ##########################################################################################################
    # Sixth step: Joint estimation of ARIMA model and outlying effects, if any

    if (n_can_out>0){
      out_joint_fit <- joint_fit(yt,can_aos,can_lss,arima_fit)
      aos <- out_joint_fit$aos
      lss <- out_joint_fit$lss
      yt_clean <- out_joint_fit$yt_clean
    } else {
      yt_clean <- yt
      aos <- lss <- w_aos <- w_lss <- NULL
    }
...
"rob_ar" <- function(yt,s){
  ...
  ##########################################################################################################
  # Determine the vector of lags to perform the regression
...
"det_oga" <- function(yt,et,pis,beg_eff,end_eff,clean){
  ...
  ##########################################################################################################
  # Compute lengths of time series and AR parameters

  nT <- length(yt)
  p <- length(pis)

  ##########################################################################################################
  # Build predictor matrix of size nT x 2nT (all times for aos and lss enter the matrix)
...
"oga" <- function(M,et,beg_eff,end_eff){
  ...
  ##########################################################################################################
  # Initialize objects and compute norms of predictors
...
"sarima_fit" <- function(yt,yt_clean){
  ...
  if (s>1) {
    spec_arima <- SLBDD::sarimaSpec(yt_clean,maxorder=c(3,1,1),period=s,method="CSS")
    ...
    spec_arima <- SLBDD::arimaSpec(yt_clean,maxorder=c(3,1,1),method="CSS")
  ...
  ##########################################################################################################
  # Obtain contaminated residuals

  con_et <- residuals(forecast::Arima(yt,model=arima_fit))
...
"config_out" <- function(nT,aos_rob,aos_fit,lss_rob,lss_fit){

  ##########################################################################################################
  # Obtain candidates

  can_aos <- sort(unique(c(aos_rob,aos_fit)))
  can_lss <- sort(unique(c(lss_rob,lss_fit)))
  n_can_aos <- length(can_aos)
  n_can_lss <- length(can_lss)
  n_can_out <- n_can_aos + n_can_lss

  ##########################################################################################################
  # If there are candidates, look for the appropriate configuration

  if (n_can_out==0){
    can_aos <- can_lss <- NULL
  } else {

    ########################################################################################################
    # Configuration for consecutive LSs

    if (n_can_lss > 1){
      if (any(diff(can_lss)==1)){
        go_AO <- can_lss[which(diff(can_lss)==1)]
        can_aos <- sort(unique(c(can_aos,go_AO)))
        can_lss <- can_lss[-which(diff(can_lss)==1)]
      }
      n_can_aos <- length(can_aos)
      n_can_lss <- length(can_lss)
      n_can_out <- n_can_aos + n_can_lss
      if (n_can_lss > 1){
        if (any(diff(can_lss)==2)){
          go_AO <- can_lss[which(diff(can_lss)==2)]
          go_AO <- c(go_AO,go_AO+1)
          can_aos <- sort(unique(c(can_aos,go_AO)))
          can_lss <- can_lss[-which(diff(can_lss)==2)]
        }
        n_can_aos <- length(can_aos)
        n_can_lss <- length(can_lss)
        n_can_out <- n_can_aos + n_can_lss
      ...
      if (n_can_lss > 1){
        if (any(diff(can_lss)==3)){
          go_AO <- can_lss[which(diff(can_lss)==3)]
          go_AO <- c(go_AO,go_AO+1,go_AO+2)
          can_aos <- sort(unique(c(can_aos,go_AO)))
          can_lss <- can_lss[-which(diff(can_lss)==3)]
        }
        n_can_aos <- length(can_aos)
        n_can_lss <- length(can_lss)
        n_can_out <- n_can_aos + n_can_lss
    ...
    if (n_can_lss > 0){
      if (n_can_out>2){
        min_out <- min(can_aos,can_lss)
        M <- matrix(0,nrow=nT-min_out+1,ncol=n_can_out)
        for (i in 1:n_can_aos){M[can_aos[i]-min_out+1,i] <- 1}
        for (i in (n_can_aos+1):n_can_out){
          M[(can_lss[i-n_can_aos]-min_out+1):(nT-min_out+1),i] <- 1}
        are_lc <- caret::findLinearCombos(M)
        n_are_lc <- length(are_lc$linearCombos)
        if (n_are_lc > 0){
          elim_col <- NULL
          for (i in 1:n_are_lc){
            are_lc_col <- are_lc$linearCombos[[i]]
            elim_col <- c(elim_col,are_lc_col[-which.max(colSums(M[,are_lc_col]))])
          }
          elim_col <- sort(unique(elim_col))
          elim_col_aos <- elim_col[elim_col<=n_can_aos]
          elim_col_lss <- elim_col[elim_col>n_can_aos]-n_can_aos
          can_aos <- can_aos[-elim_col_aos]
          can_lss <- can_lss[-elim_col_lss]
        }
      }
    }
  }
  ...
  return(list("can_aos"=can_aos,"can_lss"=can_lss))

}

"joint_fit" <- function(yt,can_aos,can_lss,arima_fit){
  ...
  ##########################################################################################################
  # Define cutoff and ARIMA model to fit

  c_val <- sqrt(qchisq(.95^(1/nT),df=1))
  ...
  while (fit=="Yes"){
    ...
    se_w_est <- sqrt(abs(as.numeric(diag(vcov(con_sarima_end)))[(n_par+1):(n_par+n_out)])) # Standard deviations
    t_test <- w_est / se_w_est # t-statistics
    del <- which.min(abs(t_test)) # Index of minimum absolute t-statistic
    if (abs(t_test[del]) > c_val){
      fit <- "No"
      if (n_aos>0){
        aos_end <- matrix(0,nrow=n_aos,ncol=3)
        aos_end[,1] <- aos
        aos_end[,2] <- w_est[1:n_aos]
        aos_end[,3] <- t_test[1:n_aos]
      } else {
        aos_end <- NULL
      }
      if (n_lss>0){
        lss_end <- matrix(0,nrow=n_lss,ncol=3)
        lss_end[,1] <- lss
        lss_end[,2] <- w_est[(n_aos+1):n_out]
        lss_end[,3] <- t_test[(n_aos+1):n_out]
      } else {
        lss_end <- NULL
      }
    } else {
...
"db_hom_oga" <- function(Y,s=NULL){
  ...
  if (run == "Yes"){
    ...
    parallel::clusterExport(cl,"config_out",environment())
